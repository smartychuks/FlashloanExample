...
describe("Deploy a Flash Loan", function() {
  it("Should take a flash loan and be able to return it", async function () {
    ...
    // To impersonate the DAI_WHALE account so as to be able to send transactions from that account
    ...
    await token
      .connect(signer)
      .transfer(_flashLoanExample.address, BALANCE_AMOUNT_DAI);// send 2000 DAI from DAI_WHALE

      const tx = await _flashLoanExample.createFlashLoan(DAI, 1000);// borrow 1000 DAI in a flash Loan from aave
      await tx.wait();
      const remainingBalance = await token.balanceOf(_flashLoanExample.address);//check balance of DAI in flash loan contract
